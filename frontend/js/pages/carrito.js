    ...
    <ul class="max-h-[calc(100vh-220px)] space-y-3 overflow-y-auto scrollbar-premium pr-1">
      ${items.map((v) => `
        <li class="anim-fade-in flex items-center gap-4 rounded-xl border border-border/50 bg-card p-4 shadow-card" data-slug="${escapeHtml(v.id)}">
          <a href="${hrefFromApp("/vehiculos/" + (v.id))}" class="relative h-16 w-24 shrink-0 overflow-hidden rounded-lg bg-secondary">
            <img src="${escapeHtml(assetPath(v.imagenes[0] ?? ""))}" alt="" class="h-full w-full object-cover" />
          </a>
          <div class="min-w-0 flex-1">
            <p class="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">${escapeHtml(v.marca)}</p>
            <a href="${hrefFromApp("/vehiculos/" + (v.id))}" class="block truncate text-sm font-semibold text-foreground hover:underline">${escapeHtml(v.modelo)}</a>

        ...
        <ul class="mt-4 space-y-3">
          ${o.items.map((it) => `
            <li class="flex items-center gap-4">
              <a href="${hrefFromApp("/vehiculos/" + (it.vehicle.id))}" class="relative h-14 w-20 shrink-0 overflow-hidden rounded-lg bg-secondary">
                <img src="${escapeHtml(assetPath(it.vehicle.imagenes[0] ?? ""))}" alt="" class="h-full w-full object-cover" />
              </a>
              <div class="min-w-0 flex-1">
                <p class="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">${escapeHtml(it.vehicle.marca)}</p>
                <a href="${hrefFromApp("/vehiculos/" + (it.vehicle.id))}" class="block truncate text-sm font-semibold text-foreground hover:underline">${escapeHtml(it.vehicle.modelo)}</a>
...
async function iniciar() {
  ...
  if (!auth.isAuthenticated) {
    window.location.href = hrefFromApp("/login") + "?redirect=" + encodeURIComponent("/pedidos")
    return
  }

  ...
  <div class="min-h-screen bg-background">
    <div class="grid min-h-screen lg:grid-cols-2">
      <div class="relative hidden overflow-hidden lg:block">
        <img src="${assetPath("/vehicles/porsche-911-carrera/1.jpg")}" alt="" class="absolute inset-0 h-full w-full object-cover" />
        <div class="absolute inset-0 bg-gradient-to-t from-background via-background/40 to-transparent"></div>
        <div class="relative z-10 flex h-full flex-col justify-end p-10">
          <a href="${hrefFromApp("/")}" class="absolute left-10 top-10 flex items-center gap-2.5" aria-label="Ir al inicio">
      ...
      <div class="flex min-h-screen items-center justify-center px-4 py-16 sm:px-6">
        <form id="login-form" class="reveal is-visible w-full max-w-md" novalidate>
          <a href="${hrefFromApp("/")}" class="mb-8 flex items-center gap-2.5 lg:hidden" aria-label="Ir al inicio">
            <span class="flex h-9 w-9 items-center justify-center rounded-xl bg-primary text-primary-foreground shadow-lg shadow-primary/10">
              ${icon("Gauge", "h-5 w-5", 2.2)}
            </span>
            <span class="flex-col items-start leading-none flex">
          ...
          <div class="mt-8 space-y-5">
            ...
            <p class="text-center text-sm text-muted-foreground">
              ¿No tienes cuenta? <a href="${hrefFromApp("/registro")}${redirect !== "/" ? "?redirect=" + encodeURIComponent(redirect) : ""}" class="font-medium text-[var(--signature)] hover:underline">Crear cuenta</a>
            </p>
          </div>
        </form>
      </div>

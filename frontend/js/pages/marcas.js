...
function brandCardMarkup(marca, index) {
  return `
  <a href="${hrefFromApp("/marcas/" + (marca.slug))}" class="reveal group relative overflow-hidden rounded-2xl border border-border/50 bg-card shadow-card transition-all duration-500 hover:border-border hover:shadow-card-hover" style="--reveal-delay: ${Math.min(index * 0.04, 0.4)}s">

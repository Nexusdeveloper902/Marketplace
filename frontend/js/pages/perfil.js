...
async function iniciar() {
  ...
  if (!auth.isAuthenticated) {
    window.location.href = hrefFromApp("/login") + "?redirect=" + encodeURIComponent("/perfil")
    return
  }
    ...
    <div class="reveal border-b border-border/40 pb-10">
      ...
      <div class="mt-5 flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-wrap items-center gap-3">
          ${user.role === "ADMIN" ? `
            <a href="${hrefFromApp("/admin")}" class="inline-flex items-center justify-center gap-2 rounded-xl border border-border/70 px-4 py-2.5 text-sm font-medium text-muted-foreground transition-colors hover:text-foreground">
    ...
    <div class="mt-10 grid grid-cols-1 gap-8 lg:grid-cols-3">
      ...
      <section class="reveal rounded-2xl border border-border/50 bg-card p-6 shadow-card lg:col-span-2 lg:p-7">
        <div class="flex items-center justify-between">
          ...
          <a href="${hrefFromApp("/pedidos")}" class="inline-flex items-center gap-1.5 text-sm font-medium text-muted-foreground transition-colors hover:text-foreground">
        ...
        ${!orders.length ? `
          <div class="flex flex-col items-center justify-center py-12 text-center">
            ...
            <a href="${hrefFromApp("/marketplace")}" class="mt-4 inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground transition-opacity hover:opacity-90">
    ...
    <section class="reveal mt-8 rounded-2xl border border-border/50 bg-card p-6 shadow-card lg:p-7">
      ...
      <div class="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ${comprados.map((v) => {
          ...
          return `
          <a href="${hrefFromApp("/vehiculos/" + (escapeHtml(v.id)))}" class="group overflow-hidden rounded-xl border border-border/50 transition-colors hover:border-border">
  ...
  main.querySelector("#perfil-logout").addEventListener("click", async () => {
    await auth.logout()
    window.location.href = hrefFromApp("/")
  })
  hydrateReveals(main)
}

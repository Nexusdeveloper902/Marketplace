...
function heroMarkup(v) {
  ...
  <section class="relative flex min-h-[100svh] items-end overflow-hidden" id="hero">
    <div class="absolute inset-0 z-0 will-parallax" data-parallax="80" id="hero-img-wrap">
      <img src="${escapeHtml(assetPath(v.imagenes[0]))}" alt="${escapeHtml(v.marca)} ${escapeHtml(v.modelo)}" class="h-full w-full object-cover" fetchpriority="high" id="hero-img" />
    ...
    <div class="relative z-10 mx-auto w-full max-w-7xl px-4 pb-20 sm:px-6 sm:pb-28 lg:px-8 lg:pb-32" id="hero-text">
      <div class="max-w-2xl">
        ...
        <div class="reveal is-visible mt-10" style="--reveal-delay: 0.45s">
          <a href="${hrefFromApp("/marketplace")}" class="group inline-flex items-center gap-3 rounded-xl bg-primary px-7 py-4 text-sm font-semibold text-primary-foreground transition-all duration-300 hover:gap-4 hover:shadow-[0_12px_40px_-8px_oklch(0.98_0_0/0.35)] active:scale-[0.98]">
  ...
  <section class="mx-auto max-w-7xl px-4 py-24 sm:px-6 sm:py-32 lg:px-8 lg:py-40">
    <div class="flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-end">
      ...
      <div class="reveal" style="--reveal-delay: 0.1s">
        <a href="${hrefFromApp("/marketplace")}" class="group inline-flex items-center gap-2 text-sm font-semibold text-foreground transition-colors hover:text-muted-foreground">
...
function showcaseMarkup(v) {
  ...
  <section class="relative flex min-h-[90svh] items-center overflow-hidden border-y border-border/40" id="showcase">
    <div class="absolute inset-0 z-0" data-parallax="60" id="showcase-img">
      <img src="${escapeHtml(assetPath(v.imagenes[1] ?? v.imagenes[0]))}" alt="${escapeHtml(v.marca)} ${escapeHtml(v.modelo)}" class="h-full w-full object-cover" loading="lazy" />
    ...
    <div class="relative z-10 mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
      <div class="max-w-xl">
        ...
        <div class="reveal mt-10" style="--reveal-delay: 0.4s">
          <a href="${hrefFromApp("/vehiculos/" + (v.id))}" class="group inline-flex items-center gap-3 rounded-xl border border-border/70 bg-background/40 px-7 py-3.5 text-sm font-semibold text-foreground backdrop-blur-md transition-all duration-300 hover:bg-background/60 hover:gap-4 active:scale-[0.98]">

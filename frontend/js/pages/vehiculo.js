...
function galeriaMarkup() {
  ...
  <div class="flex flex-col gap-3" id="v-galeria">
    <div class="relative aspect-[16/10] overflow-hidden rounded-2xl border border-border/50 bg-secondary shadow-card group">
      <img src="${escapeHtml(assetPath(inicial))}" alt="${escapeHtml(vehiculo.marca)} ${escapeHtml(vehiculo.modelo)}" class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" id="v-img-activa" />
    ...
    <div class="flex gap-3 overflow-x-auto pb-1" id="v-miniaturas">
      ${imagenes.map((img, i) => `
        <button data-idx="${i}" class="miniatura relative h-20 w-32 shrink-0 overflow-hidden rounded-xl border ${i === 0 ? "border-foreground/60" : "border-border/50"} bg-secondary transition-all" aria-label="Imagen ${i + 1}">
          <img src="${escapeHtml(assetPath(img))}" alt="" class="h-full w-full object-cover" />
        </button>`).join("")}
    </div>
    <div id="v-viewer"></div>
  </div>`
    ...
    viewer.innerHTML = `
      <div class="fixed inset-0 z-[100] flex items-center justify-center bg-background/95 backdrop-blur-xl anim-fade-in" id="v-viewer-overlay">
        <div class="anim-pop-in relative">
          <img src="${escapeHtml(assetPath(img))}" alt="Imagen ampliada" class="max-h-[85vh] max-w-[90vw] rounded-2xl border border-border/50 object-contain shadow-card-hover" />
...
function renderAcciones() {
  ...
  box.innerHTML = `
    <div class="flex flex-col gap-3">
      ...
      ${estaComprado ? `
        <a href="${hrefFromApp("/garaje")}" class="rounded-xl border border-border bg-card px-6 py-4 text-center text-sm font-semibold text-foreground transition-colors hover:bg-accent">Ver en mi garaje</a>` : ""}
      ${estaEnCarrito && !estaComprado ? `
        <a href="${hrefFromApp("/carrito")}" class="rounded-xl border border-border bg-card px-6 py-4 text-center text-sm font-semibold text-foreground transition-colors hover:bg-accent">Ver carrito y finalizar compra</a>` : ""}
    </div>
...
async function iniciar() {
  ...
  try {
    ...
    window.location.href = hrefFromApp("/marketplace")
    return
  }
  ...
  <section class="mx-auto max-w-7xl px-4 py-10 pb-24 sm:px-6 lg:px-8 lg:py-14">
    <a href="${hrefFromApp("/marketplace")}" class="inline-flex items-center gap-2 text-sm text-muted-foreground transition-colors hover:text-foreground">
